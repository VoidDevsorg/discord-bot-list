...
app.get("/:code/delete", global.checkAuth, async (req, res) => {
  ...
  if (!dde)
    return res.redirect(
      "/uptime/links?error=true&message=There is no such site in the system."
    );
  uptimedata.findOne(
    {
      code: req.params.code,
    ...
    async function (err, docs) {
      if (docs.userID != req.user.id)
        return res.redirect(
          "/uptime/links?error=true&message=The link you tried to delete does not belong to you."
        );
      res.redirect(
        "/uptime/links?success=true&message=The link has been successfully deleted from the system."
      );
