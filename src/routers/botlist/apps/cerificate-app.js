...
app.post("/certification/apply", global.checkAuth, async (req, res) => {
  ...
  if (!botdata)
    return res.redirect("/error?code=404&message=You entered invalid bot id.");
  ...
  if (botdata.certificate === "Certified")
    return res.redirect("/error?code=401&message=This bot already certified.");
  let findBot = await apps.findOne({ botID: rBody["bot"] });
  if (findBot)
    return res.redirect(
      "/error?code=401&message=This bot application already applied."
    );
  new apps({
    botID: rBody["bot"],
    future: rBody["future"],
  ...
  res.redirect(
    "/bots?success=true&message=Certificate application applied.&botID=" +
      rBody["bot"]
  );
