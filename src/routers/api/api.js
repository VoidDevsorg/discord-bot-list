...
app.post("/api/search", async (req, res) => {
  ...
  let data = await bot
    .filter(
      (d) =>
        d.status == "Approved" &&
        d.username.toLowerCase().includes(key.toLowerCase())
    )
...
app.post("/api/search/servers", async (req, res) => {
  ...
  let data = await server
    .filter(
      (d) =>
        global.clientSL.guilds.cache.get(d.id) &&
        d.name.toLowerCase().includes(key.toLowerCase())
    )
    .sort(
      (a, b) =>
        global.clientSL.guilds.cache.get(b).memberCount -
        global.clientSL.guilds.cache.get(a).memberCount
    );
  res.json({
    status: true,
    data: data,
