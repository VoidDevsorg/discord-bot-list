...
app.get("/view/:code", global.checkAuth, async (req, res) => {
  ...
  if (!koddata)
    return res.redirect(
      "/codes/list?error=true&message=You entered an invalid code."
    );
  ...
  if (koddata.codeCategory == "subs") {
    if (
      !client.guilds.cache
        .get(config.server.id)
        .members.cache.get(req.user.id)
        .roles.cache.get(config.server.roles.codeshare.substructure)
    )
      return res.redirect(
        "/error?code=403&message=You is not competent to do this."
      );
  ...
  if (koddata.codeCategory == "5invites") {
    if (
      !client.guilds.cache
        .get(config.server.id)
        .members.cache.get(req.user.id)
        .roles.cache.get(config.server.roles.codeshare.fiveInvite)
    )
      return res.redirect(
        "/error?code=403&message=You is not competent to do this."
      );
  ...
  if (koddata.codeCategory == "10invites") {
    if (
      !client.guilds.cache
        .get(config.server.id)
        .members.cache.get(req.user.id)
        .roles.cache.get(config.server.roles.codeshare.tenInvite)
    )
      return res.redirect(
        "/error?code=403&message=You is not competent to do this."
      );
  ...
  if (koddata.codeCategory == "15invites") {
    if (
      !client.guilds.cache
        .get(config.server.id)
        .members.cache.get(req.user.id)
        .roles.cache.get(config.server.roles.codeshare.fifteenInvite)
    )
      return res.redirect(
        "/error?code=403&message=You is not competent to do this."
      );
  ...
  if (koddata.codeCategory == "20invites") {
    if (
      !client.guilds.cache
        .get(config.server.id)
        .members.cache.get(req.user.id)
        .roles.cache.get(config.server.roles.codeshare.twentyInvite)
    )
      return res.redirect(
        "/error?code=403&message=You is not competent to do this."
      );
