...
exports.run = async (client, message, args) => {
  ...
  if (cooldown - (Date.now() - lastDaily) > 0) {
    ...
    const correctButton = new Discord.MessageEmbed()
      ...
      .setDescription(
        `You have successfully bumped for server **${message.guild.name}**.`
      )
    ...
    message.channel
      .send({
        embed: controlEmbed,
        buttons: [sorgu, sorgu2, sorgu3].sort(() => Math.random() - 0.5),
      })
      .then(async (msg) => {
        ...
        collector.on("collect", async (b) => {
          if (b.id == kod1) {
            let findServerr = await serverData.findOne({
              id: message.guild.id,
            });
            ...
            if (cooldown - (Date.now() - lastDailyy) > 0)
              return msg
                ...
                .then(
                  await msgError(
                    "This command is used only once every 30 minutes.",
                    { channel: message.channel }
                  )
                );
            msg
              .delete()
              .then(
                message.channel.send({ embed: correctButton, buttons: [web] })
              );
            ...
            msg
              .delete()
              .then(
                message.channel.send({ embed: incorrectButton, buttons: [web] })
              );
          }
        });
      });
...
function msgError(msg, { channel }) {
  channel.send(
    new Discord.MessageEmbed()
      .setAuthor(
        global.clientSL.user.username,
        global.clientSL.user.avatarURL()
      )
